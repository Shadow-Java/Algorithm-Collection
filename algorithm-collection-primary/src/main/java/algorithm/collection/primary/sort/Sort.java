...
public class Sort {
    ...
    /**
     * 插入排序：将待排序的元素不断插入已排序的序列中，直到所有元素都插入完毕为止
     *
     * 假设我们有以下一组数字需要排序：[3, 5, 1, 4, 2]。
     *
     * 首先将第一个数字3看作是已经排序的序列，因为只有一个数字，所以它是有序的。
     * 然后将第二个数字5插入已排序的序列中，因为5大于3，所以5放在3的后面，序列变为[3, 5]。
     * 将第三个数字1插入已排序的序列中，因为1小于3，所以1放在3的前面，序列变为[1, 3, 5]。
     * 将第四个数字4插入已排序的序列中，因为4大于3，小于5，所以4放在3和5之间，序列变为[1, 3, 4, 5]。
     * 最后将最后一个数字2插入已排序的序列中，因为2小于3，所以2放在1的前面，序列变为[1, 2, 3, 4, 5]。
     *
     *
