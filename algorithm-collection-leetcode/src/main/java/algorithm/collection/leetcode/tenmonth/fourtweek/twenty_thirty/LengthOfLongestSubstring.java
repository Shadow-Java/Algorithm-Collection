...
public class LengthOfLongestSubstring {
     ...
     *   的右端点，如果发现 s
     * ′
     *   其中一种字母的出现次数大于 p 的这种字母的出现次数，则右移 s
     * ′
     *   的左端点。如果发现 s
     * ′
     *   的长度等于 p 的长度，则说明 s
     * ′
     *   的每种字母的出现次数，和 p 的每种字母的出现次数都相同（如果出现次数 s
     * ′
     *   的小于 p 的，不可能长度一样），那么 s
     * ′
     *   是 p 的异位词。
     * 方法一：定长滑窗
     ...
     * 证明：内层循环结束后，s
     * ′
     *   的每种字母的出现次数，都小于等于 p 的每种字母的出现次数。如果 s
     * ′
     *   的其中一种字母的出现次数比 p 的小，那么 s
     * ′
     *   的长度必然小于 p 的长度。所以只要 s
     * ′
     *   的长度等于 p 的长度，就说明 s
     * ′
     *   的每种字母的出现次数，和 p 的每种字母的出现次数都相同，s
     * ′
     *   是 p 的异位词，把 s
     * ′
     *   左端点下标加入答案。
